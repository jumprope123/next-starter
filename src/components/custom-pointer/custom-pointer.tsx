...
/**
 * iPadOS(26 이전) 트랙패드 스타일 커스텀 포인터 — **마우스 이벤트 전용**.
 *
 * - 평소: 커서를 따라다니는 반투명 원형 도트
 * - 인터랙션 요소 위: 요소를 감싸는 라운드 하이라이트로 변형 + 커서 방향으로 살짝 끌려가는 시차 효과
 * - 텍스트 입력 위: I-beam 캐럿 바
 * - 터치·펜: 표시하지 않는다. 모바일/터치 기기는 `(hover: hover) and (pointer: fine)` 미디어
 *   게이트에서 아예 비활성(리스너 미등록·네이티브 커서 유지)이고, 하이브리드 기기에선
 *   pointerType 'mouse' 가 아닌 이벤트와 touch 이벤트 감지 시 즉시 숨긴다 — 터치를 'mouse' 로
 *   잘못 보고하는 브라우저(인앱 웹뷰·DeX 등)까지 touch 이벤트 기반으로 확실히 해제.
 *
 * 활성화 시 `<html>` 에 CUSTOM_POINTER_ACTIVE_CLASS 를 붙여 네이티브 커서를 숨긴다
 * (스타일은 `@/styles/custom-pointer.css`).
...
export function CustomPointer() {
  ...
  useEffect(() => {
    ...
    // 스크롤 중엔 스냅/캐럿 판정을 멈추고 기본 도트로 고정한다 — 멎으면 타이머가 재판정
    let scrolling = false;
    ...
    const retarget = (node: EventTarget | null) => {
      // 스크롤 중에는 어떤 대상에도 스냅하지 않는다 — 지나가는 요소마다 하이라이트가 변하는 것을 방지
      if (scrolling) {
        mode = 'dot';
        snapTarget = null;
        return;
    ...
    // Chromium 은 터치에서 합성된 이벤트에 sourceCapabilities.firesTouchEvents 를 표시한다 —
    // 첫 touchstart 도착 전이나 억제 시간이 지난 뒤의 잘못 보고된 'mouse' 이벤트까지 걸러낸다
    const firesTouchEvents = (event: PointerEvent) =>
      (event as PointerEvent & { sourceCapabilities?: { firesTouchEvents?: boolean } }).sourceCapabilities
        ?.firesTouchEvents === true;

    // 마우스가 아닌 모든 입력(터치·펜)과 터치 유래로 의심되는 이벤트는 표시 대상에서 제외한다
    const isTouchLike = (event: PointerEvent) =>
      event.pointerType !== 'mouse' || firesTouchEvents(event) || performance.now() < touchSuppressUntil;
    ...
    const handleScroll = () => {
      // 스크롤 중에는 항상 기본 도트 — 스냅/캐럿을 즉시 해제하고 판정을 잠근다
      scrolling = true;
      mode = 'dot';
      snapTarget = null;
