export const CUSTOM_POINTER_ELEMENT_ID = 'app-custom-pointer';

/** `<html>` 에 붙어 네이티브 커서를 숨기고 커스텀 포인터를 활성화하는 클래스 */
export const CUSTOM_POINTER_ACTIVE_CLASS = 'app-custom-pointer-active';
 ...
 * 클릭이 일으키는 DOM 변경(장바구니 담기 등)은 이벤트 처리 이후에 커밋되므로,
 * pointerup 시점의 단발 판정으로는 새 레이아웃을 반영할 수 없다.
 */
export const CUSTOM_POINTER_CLICK_RECHECK_MS = 250;

/**
 * 마지막 scroll 이벤트 후 이 시간이 지나야 스크롤이 멎은 것으로 보고 스냅 판정을 재개한다(ms).
 * 스크롤 중에는 커서 밑을 지나가는 요소마다 하이라이트가 튀지 않도록 항상 기본 도트로 고정한다.
 */
export const CUSTOM_POINTER_SCROLL_SETTLE_MS = 150;
